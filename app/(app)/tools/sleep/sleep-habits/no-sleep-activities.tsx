...
export default function NoSleepActivities() {
  const { t } = useTranslation('tools');
  const { toolsTranslationKeys } = useTranslationKeys();
  const { data: sleepActivity, refetch } = useSleepActivitiesByType(SleepActivityType.NO_SLEEP);
  const [favoriteItems, setFavoriteItems] = useState<StarListItem[]>([]);

  useEffect(() => {
    const elements = t(toolsTranslationKeys.SLEEP.subcategories.SLEEP_HABITS.noSleepActivities, {
      returnObjects: true,
    });
  ...
  const handleDatabaseUpdate = async (updatedItems: Set<string>) => {
    if (!sleepActivity) {
      await sleepActivitiesRepository.createSleepActivity({
        type: SleepActivityType.NO_SLEEP,
      ...
      await sleepActivitiesRepository.updateSleepActivity(SleepActivityType.NO_SLEEP, {
        favorites: Array.from(updatedItems),
      });
    }
  };
    ...
    <>
      <Screen
        headerProps={{
          ...
          onRightPress: () => router.push('/tools/sleep/sleep-habits/info?tool=no-sleep-activities'),
        }}
        footerProps={{
          ...
          listHeaderComponent={
            <>
              <Typography paddingBottom={8} preset='default'>
                {t(toolsTranslationKeys.SLEEP.subcategories.SLEEP_HABITS.noSleepActivitiesDescription)}
              </Typography>
            </>
          }
          items={favoriteItems}
